...
import com.example.sipantau.model.ProgresPertanian
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import retrofit2.awaitResponse

class ProgresPertanianRepository(private val context: Context) {

    private val dao = AppDatabase.getDatabase(context).progresPertanianDao()
     ...
     */
    suspend fun getProgres(): List<ProgresPertanianEntity> = withContext(Dispatchers.IO) {

        val local = dao.getAll()

        if (isOnline() && token != null) {
            try {
                val resp = ApiClient.instance.getProgresPertanian(token!!).awaitResponse()
    ...
    suspend fun insertProgres(
        entity: ProgresPertanianEntity
    ): Pair<Boolean, Long> = withContext(Dispatchers.IO) {

        try {
            if (isOnline() && token != null) {

                val resp = ApiClient.instance.createProgresPertanian(
                    token!!,
                ...
                if (resp.isSuccessful && resp.body() != null) {
                    ...
                    val saved = ProgresPertanianEntity(
                        local_id = 0L,
                        server_id = created.id_pantau_progres_pertanian,
        ...
        try {
            if (serverId != null && isOnline() && token != null) {
                ...
                val resp = ApiClient.instance
                    .deleteProgresPertanian(token!!, serverId)
                    .awaitResponse()
     ...
     * 🔹 MAPPING API → ENTITY
     */
    private fun apiToEntity(p: ProgresPertanian) =
        ProgresPertanianEntity(
            local_id = 0L,
            server_id = p.id_pantau_progres_pertanian,
