...
import com.example.sipantau.model.ProgresIndustriDigital
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import retrofit2.awaitResponse

class ProgresIndustriDigitalRepository(private val context: Context) {

    private val dao = AppDatabase.getDatabase(context).ProgresIndustriDigitalDao()
     ...
     */
    suspend fun getProgres(): List<ProgresIndustriDigitalEntity> = withContext(Dispatchers.IO) {

        val local = dao.getAll()

        if (isOnline() && token != null) {
            try {
                val resp = ApiClient.instance.getProgresIndustriDigital(token!!).awaitResponse()
    ...
    suspend fun insertProgres(
        entity: ProgresIndustriDigitalEntity
    ): Pair<Boolean, Long> = withContext(Dispatchers.IO) {

        try {
            if (isOnline() && token != null) {

                val resp = ApiClient.instance.createProgresIndustriDigital(
                    token!!,
                ...
                if (resp.isSuccessful && resp.body() != null) {
                    ...
                    val saved = ProgresIndustriDigitalEntity(
                        local_id = 0L,
                        server_id = created.id_lapor_progres_industri_digital,
        ...
        try {
            if (serverId != null && isOnline() && token != null) {
                ...
                val resp = ApiClient.instance
                    .deleteProgresIndustriDigital(token!!, serverId)
                    .awaitResponse()
     ...
     * 🔹 MAPPING API → ENTITY
     */
    private fun apiToEntity(p: ProgresIndustriDigital) =
        ProgresIndustriDigitalEntity(
            local_id = 0L,
            server_id = p.id_lapor_progres_industri_digital,
