...
class PelaporanPmlAdapter(
    private var items: List<DisplayLaporanPml>,
    private val onDeleteClick: (DisplayLaporanPml) -> Unit,
    private val onSendClick: (DisplayLaporanPml) -> Unit
) : RecyclerView.Adapter<PelaporanPmlAdapter.VH>() {
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        with(holder.b) {
            ...
            // Text fields
            judul.text = item.nama_kegiatan_detail_proses.ifEmpty { "Laporan PML" }
            // Tambahkan hari pada created_at
            tanggal.text = "${getDayName(item.created_at ?: "")}, ${item.created_at ?: "-"}"
            textView.text = item.resume
            kegiatan.text = item.nama_kegiatan_detail_proses.ifEmpty { "" }
    ...
    fun update(newList: List<DisplayLaporanPml>) {
        items = newList
