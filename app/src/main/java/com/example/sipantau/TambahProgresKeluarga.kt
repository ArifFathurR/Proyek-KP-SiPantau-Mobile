...
class TambahProgresKeluarga : AppCompatActivity() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@TambahProgresKeluarga, "Error memuat Sub-SLS: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private suspend fun showSubslsDropdown(list: List<SubslsLocalEntity>) {
        withContext(Dispatchers.Main) {
            ...
            val adapter = ArrayAdapter(this@TambahProgresKeluarga, android.R.layout.simple_dropdown_item_1line, names)
    ...
    private fun simpanProgres() {
        ...
        val entity = ProgresKeluargaEntity(
            local_id = 0L,
            server_id = null,
